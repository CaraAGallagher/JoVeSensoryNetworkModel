...
; Gallagher, C.A., Roeleke, M., and Jeltsch, F.
; Unveiling complex foraging strategies through simulation: Examining mobile sensory network formation and benefits in common noctule bats.
...
globals [
  ...
  consecutive-networking-ts    ; list of the number of consecutive timesteps spent in a network [ticks]
  perc-ts-networking           ; combined list tracking whether bats are (1) or are not (0) engaged in a network per timestep
...
bats-own [
  ...
  ; for animation paper
  ID-prev-ts              ; network id from one timestep used to compare if network continues in the next timestep
  networking-ticker       ; ticker to keep track of how long bats consecutively remain in a network
...
to setup
  ...
  set consecutive-networking-ts []
  set perc-ts-networking []
  if track-bats? = true [
   set track-list []
   set track-outputs []
  ]
...
to bats-move
  ...
  ifelse flying-towards-food = false [
    ...
    let avg-x (((item 0 attract-vect) * (attract-strength * soc-mod)) + ((item 0 align-vect) * (align-strength * soc-mod)) + ((item 0 avoid-vect) * (avoid-strength * soc-mod)) + ((item 0 rw-vect) * rw-strength))
    let avg-y (((item 1 attract-vect) * (attract-strength * soc-mod)) + ((item 1 align-vect) * (align-strength * soc-mod)) + ((item 1 avoid-vect) * (avoid-strength * soc-mod)) + ((item 1 rw-vect) * rw-strength))
...
to update-monitors
  ...
  set-current-plot "Ticks in networks"
  if length consecutive-networking-ts > 1 [ set-plot-x-range 1 max consecutive-networking-ts ]
  histogram consecutive-networking-ts
end
...
to calculate-network-size
  ...
  let num 5
  let net-id [network-id] of bat num

  ask bats [ pu ]
  ; update colors
 ; ask bats [ifelse food-found = true [ set color white ] [ set color (min [who] of bats with [network-ID = [network-ID] of myself] + 10 )]]
  ask bats [ifelse food-found = true [ set color white ] [ if [food-found] of bat num = false and [network-ID] of bat num != 0 [ if network-id = net-id [pd] ]]]
end
...
to collect-outputs
  ...
  if track-bats? = true [
    ...
    [
      ...
      ; record number of consecutive timesteps spent networking
      let ID-this-ts network-ID
      if ID-this-ts != 0 and ID-prev-ts != 0 [ set networking-ticker networking-ticker + 1 ]
      if ID-this-ts = 0 and ID-prev-ts != 0 [
        set consecutive-networking-ts lput networking-ticker consecutive-networking-ts
        set networking-ticker 0
      ]
      set ID-prev-ts ID-this-ts

      ; keep track of amount of total time spent in versus out of networks
      let t 0
      if network-ID != 0 [ set t 1 ]
      set perc-ts-networking lput t perc-ts-networking
    ]
  ]
...
<experiments>
  <experiment name="JoVE Experiment" repetitions="500" runMetricsEveryStep="false">
    ...
    <metric>time-to-all-food</metric>
